...
- La tabla `orders` no tiene una columna dedicada a la fecha de cancelación: como toda orden en esta respuesta tiene `status` `5`, `updatedAt` **es** el momento en que se canceló (la última escritura sobre la orden es justo el cambio de estado a cancelada).
...
<ResponseField name="data" type="object[]">
  ...
  <Expandable title="Propiedades de cada orden">
    ...
    <ResponseField name="updatedAt" type="string">
      Fecha y hora en que la orden fue cancelada, en formato ISO 8601. La tabla `orders` no guarda un campo dedicado para esto: como toda orden de esta respuesta tiene `status` `5`, `updatedAt` refleja el momento del cambio a cancelada.
    </ResponseField>
...
<ResponseExample>
`updatedAt` es la fecha de cancelación de la orden (`2026-08-28T23:55:04.000Z` en el ejemplo).
